...
bypass_ratio = (nm * (1 + f) * T_x * (1 - T_t) - T_r * (T_c - 1)) / (
    T_r * (T_f - 1)
)  # [-]
...
ht6m = ((cp * T0) / (1 + bypass_ratio + f)) * (
    (1 + f) * T_t * T_x + bypass_ratio * T_r * T_f
)  # [J/kg]
Tt6m = ht6m / cp  # [K]
M15 = M5  # [-]
...
K1 = (1 + y * M5**2 + A_ratio * (1 + y * M15**2)) * (
    (1 + A_ratio) ** -1
)  # Non-dimensional parameters [-]
K2 = (((M5 / a5) + ((M15 * A_ratio) / a15)) * math.sqrt((y - 1) * cp * Tt6m)) * (
    (1 + A_ratio) ** -1
)  # [-]
K = (K1 / K2) ** 2  # [-]
